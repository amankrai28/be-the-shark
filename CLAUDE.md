...
- `data/pitches.json` — canonical dataset, **schema v2**: 784 real pitches from Seasons 1–5 (428 deals / 356 no-deals). S1–3 records verified against public episode tables (89%); all S4–5 records parsed directly from them with round-trip checks. 79 deals carry `hasDebt`/`debtAmountLakhs`; royalty/split structures noted in `dealNote`. Symbolic-ask stunt pitches are excluded by policy (Cocofit ₹5, Watt ₹101, Dharaksha ₹1,250, Off Mint ₹10, Dhruv Vidyut time-for-equity) — real episodes but unscorable on amount/equity. S4–5 records have `city: "India"` and null `yearsInBusiness`/`salesChannel` (not in the public tables — UI hides them).
- **Daily schedule is PINNED** in `src/data/dailyOrder.json` (maintained by `scripts/update-daily-order.mjs`): dataset growth appends to the end of the cycle and can never reshuffle past or scheduled days. A continuity test hash-locks the launch prefix (day 213 = pitch id 475).
